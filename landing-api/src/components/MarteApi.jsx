...
const MarteApi = () => {
    ...
    useEffect(() => {
    // Toma del local storage la ultima consulta realizada, si la hubo, al cargar la pagina
        const ultimasFotos = localStorage.getItem('ultimasFotos');
        if (ultimasFotos) {
        setFotos(JSON.parse(ultimasFotos));
        }
    }, []);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.photos.length > 0) {
                ...
                // Guardar la ultima consulta exitosa en el local storage
                localStorage.setItem('ultimasFotos', JSON.stringify(data.photos));
            } else {
                setFotos([
                    {
                        id: null,
                        img_src:
                            'https://img.freepik.com/vector-premium/anotando-que-encontro-telescopio-planeta-monitoreo-espacial-galaxy-star-mensaje-error-404-elemento-ilustracion-estado-vacio_22052-4888.jpg?w=2000',
                        rover: {
            ...
            <Row>
                <Col sm={6}>
                    ...
                    <fieldset enable>
                        ...
                        <Form.Label htmlFor='sol'>
                            Ingrese un valor para Sol marciano de la misión del Rover (entre 1000 y 3000)
                        </Form.Label>
                        <Form.Control
                            id='sol'
                            type='number'
